...
class ItemsService {
  ...
  async getHotDeals(size = 4, page = 1, lang = LANGS.ENG) {
    const numberOfHotDeals = 8;
    const itemsCount = await Items.countDocuments().exec();
    const rand = Math.floor(Math.random() * (itemsCount - numberOfHotDeals));
    const items = await Items
        .find()
        .skip(rand)
        .limit(Number(numberOfHotDeals))
        .lean()
        .exec();
    return this.pagination(this.languageSpecific(items, lang), size, page);
  }
